...
// {{.SetName}} is the main set structure that holds all the data
...
func NewWithSize(size int) *{{.SetName}} {
	return &{{.SetName}}{make(map[{{.SetType}}]struct{}, size)}
}

// Add includes the specified items (one or more) to the Set. The underlying
// Set s is modified. If passed nothing it silently returns.
func (s *{{.SetName}}) Add(items ...{{.SetType}}) {
...
func (s *{{.SetName}}) Remove(items ...{{.SetType}}) {
	for _, item := range items {
		delete(s.m, item)
	}
}
...
// returned.
func (s *{{.SetName}}) Pop2() ({{.SetType}}, bool) {
	for item := range s.m {
		delete(s.m, item)
		return item, true
	}
...
func (s *{{.SetName}}) Has(items ...{{.SetType}}) bool {
	has := false
	for _, item := range items {
...
func (s *{{.SetName}}) HasAny(items ...{{.SetType}}) bool {
	has := false
	for _, item := range items {
		if _, has = s.m[item]; has {
			break
...
func (s *{{.SetName}}) Clear() {
	s.m = make(map[{{.SetType}}]struct{})
}

// IsEmpty reports whether the Set is empty.
func (s *{{.SetName}}) IsEmpty() bool {
	return s.Size() == 0
}

// IsEqual test whether s and t are the same in size and have the same items.
func (s *{{.SetName}}) IsEqual(t *{{.SetName}}) bool {
...
func (s *{{.SetName}}) IsSubset(t *{{.SetName}}) bool {
	if s.Size() < t.Size() {
		return false
	}
...
func (s *{{.SetName}}) IsSuperset(t *{{.SetName}}) bool {
	return t.IsSubset(s)
}
...
func (s *{{.SetName}}) Each(f func(item {{.SetType}}) bool) {
	for item := range s.m {
		if !f(item) {
			break
		}
...
func (s *{{.SetName}}) List() []{{.SetType}} {
	v := make([]{{.SetType}}, 0, s.Size())
	for item := range s.m {
		v = append(v, item)
	}
	return v
...
func (s *{{.SetName}}) Merge(t *{{.SetName}}) {
	for item := range t.m {
		s.m[item] = keyExists
	}
}
...
func (s *{{.SetName}}) Separate(t *{{.SetName}}) {
	for item := range t.m {
		delete(s.m, item)
	}
}
...
func Union(sets ...*{{.SetName}}) *{{.SetName}} {
	maxPos := -1
	maxSize := 0
...
func Difference(set1 *{{.SetName}}, sets ...*{{.SetName}}) *{{.SetName}} {
	s := set1.Copy()
...
// given sets.
func Intersection(sets ...*{{.SetName}}) *{{.SetName}} {
	minPos := -1
...
func SymmetricDifference(s *{{.SetName}}, t *{{.SetName}}) *{{.SetName}} {
	u := Difference(s, t)
	v := Difference(t, s)
	return Union(u, v)
}
